$(document).ready(function() {
  ...
  /*Envio del formulario con Ajax para agregar backup*/
  $('#add-copias form').submit(function(e) {
    e.preventDefault();
    var informacion = $('#add-copias form').serialize();
    var metodo = $('#add-copias form').attr('method');
    var peticion = $('#add-copias form').attr('action');
    $.ajax({
      type: metodo,
      url: peticion,
      data: informacion,
      beforeSend: function() {
        $("#res-form-add-copias").html('Agregando Backup...<br><img src="assets/img/enviando.gif" class="center-all-contens">');
      },
      error: function() {
        $("#res-form-add-copias").html("Ha ocurrido un error en el sistema");
      },
      success: function(data) {
        $("#res-form-add-copias").html(data);
      }
    });
    return false;
  });
